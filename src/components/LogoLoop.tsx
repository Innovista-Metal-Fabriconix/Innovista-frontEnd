...
import styles from "../cssModules/LogoLoop.module.css";
...
export const LogoLoop = memo(
  ({
    ...
    const rootClassName = useMemo(() => {
      return [
        styles.logoloop,
        isVertical ? styles["logoloop--vertical"] : "",
    ...
    }, [disableMotion, effectiveHoverSpeed]);
    ...
    const renderLogoItem = useCallback(
      (item: LogoLoopItem, key: string) => {
        if (renderItem) {
          return (
            <li className={styles["logoloop__item"]} key={key}>
              {renderItem(item, key)}
            </li>
          );
        }
        ...
        const content = isNodeItem ? (
          <span
            className={styles["logoloop__node"]}
    ...
    const resolvedWidth = useMemo(() => {
      const cssWidth = toCssLength(width) ?? "100%";
      if (isVertical) {
        return cssWidth === "100%" ? undefined : cssWidth;
      }
      return cssWidth;
    }, [width, isVertical]);
    ...
    const containerStyle = useMemo(
      ...
      [resolvedWidth, cssVariables, style],
    );
